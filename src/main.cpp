// Copyright (c) 2017 Roman Avery
// Copyright (c) 2017 The Audium developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // audium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "audium Signed Message:\n";
...
{
    // audium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // audium: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // audium: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // audium: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // audium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // audium: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // audium: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 4/Dec/2017 North Korea says US-South Korea air combat exercise pushing region to 'brink of nuclear war'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // audium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("audiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("audiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static audiumMiner(CWallet *pwallet)
{
    printf("audiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("audium-miner");
    ...
    try { loop {
        ...
        printf("Running audiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&audiumMiner, pwallet));
}
